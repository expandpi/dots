...
yay -Syy bspwm polybar sxhkd eww dunst rofi lsd jq checkupdates-aur playerctl mpd ncmpcpp mpc picom-arian8j2-git xtitle  betterlockscreen ttf-jetbrains-mono nerd-fonts-jetbrains-mono ttf-terminus-nerd ttf-inconsolata ttf-joypixels nerd-fonts-cozette-ttf scientifica-font feh maim pamixer libwebp webp-pixbuf-loader xorg-xkill papirus-icon-theme  tmux wget neovim vim powerline python-pip thunar bat zoxide lsd tmux qt5ct qt6ct zsh xf86-video-qxl extra/qemu-audio-spice extra/spice community/spice-protocol community/spice-vdagent extra/xorg-xsetroot terminology
...
cd /tmp &&  curl -O https://blackarch.org/strap.sh
chmod +x strap.sh
sudo ./strap.sh
sudo pacman -Syu
...
cd && echo  $'/usr/bin/spice-vdagent & \nxsetroot -cursor_name left_pt' >> .xprofile 
